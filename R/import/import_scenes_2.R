...
buildCharNetwork <- function(sceneCharacters){
  if (!is.null(sceneCharacters)){
    if(nrow(sceneCharacters)>1) {
      if("name" %in% names(sceneCharacters)) {
        print("ok")
        return(tibble(from="giul",to="giul"))
      } else {
        print("characters has no names")
      }
    } else {
      print("characters is less than 2")
    }
  } else {
  ...
  print("not ok")
  return(tibble(from="",to=""))
}

listOfScenes <- eps[1,]$scenes[[1]] 
sceneCharacters <- listOfScenes[5,]$characters[[1]]

buildCharNetwork(sceneCharacters)
